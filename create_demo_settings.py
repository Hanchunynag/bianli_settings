    ...
    for y in range(height):
        ...
        for x in range(width):
            if y < 180:
                ...
                color = (45, 108, 223)
            elif 390 <= y <= 520 or 560 <= y <= 690 or 730 <= y <= 860:
                color = (255, 255, 255)
            else:
                color = (238, 242, 247)
            row.extend(color)
...
def state(page_name, title, parent_page="", parent_title=""):
    result = {
        "page_name": page_name, "raw_page_name": state_name_from_title(title),
        "page_description": f"{parent_title} to{title}" if parent_title else title,
        "last_title": title, "signature": {"title": title, "texts_any": [title]}, "merged_candidates": [],
    }
    if parent_page:
...
def step(name, key):
    return {
        ...
        "target": {
            "key": key, "text": name,
            "key_description": name, "step_prompt": name,
        },
    }
...
def transition(tid, from_page, to_page, steps):
    return {
        "transition_id": tid, "from_page": from_page, "to_page": to_page,
        "operate": "tap", "target": steps[0]["target"], "steps": steps,
    }


def operation(oid, operate, name, key, effect, component_type="Card"):
    return {
        "operation_id": oid, "created_at": "2026-07-01T00:00:00", "operate": operate,
        "target": {
            "type": component_type, "key": key, "text": name,
            "key_description": name, "step_prompt": name,
        },
        "effect": effect,
    }
...
def candidate(name, key, transition_ids=None, component_type="ListItem"):
    return {
        "candidate_id": f"key::{key}", "type": component_type, "value": key,
        "component_type": component_type, "text": name, "key": key,
        "key_description": name, "step_prompt": name, "source": "demo_generated",
        "transition_ids": transition_ids or [], "operation_ids": [],
    }


def add_transition_record(graph, from_page, to_page, name, key, component_type="ListItem"):
    tid = f"{from_page}__to__{to_page}"
    graph["transitions"].append(transition(tid, from_page, to_page, [step(name, key)]))
...
def build_large_settings_graph():
    ...
    used = {"Pages_root"}
    graph["states"]["Pages_root"] = state("Pages_root", "设置")
    catalog = [
    ...
    for category_index, (category, children) in enumerate(catalog, start=1):
        ...
        graph["states"][category_page] = state(category_page, category, "Pages_root", "设置")
        ...
        for child_index, child in enumerate(children, start=1):
            ...
            graph["states"][child_page] = state(child_page, child, category_page, category)
            ...
            if child_index in {1, 2}:
                ...
                graph["states"][detail_page] = state(detail_page, detail_title, child_page, child)
